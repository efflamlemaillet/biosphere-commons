    ...
    if [ "$category" == "Deployment" ]; then
        ...
        for name in `ss-get ss:groups | sed 's/, /,/g' | sed 's/,/\n/g' `; do 
            ...
            for i in $(echo $ids | sed 's/,/\n/g'); do
                ...
                echo "$ip    $name_safe-$i " >> /etc/hosts
                echo "$ip    $name_safe.$i " >> /etc/hosts
                if [ "$mult" == "1" ]; then
                    echo "$ip    $name_safe " >> /etc/hosts
                fi
                echo "$ip is now in /etc/hosts and known as $name_safe-$i, $name_safe.$i and maybe $name_safe if only one"
            done
        done
    else
...
populate_hosts_with_components_name_and_ips_on_vm_add(){
    ...
    for INSTANCE_NAME in $SLIPSTREAM_SCALING_VMS; do
        INSTANCE_NAME_SAFE=$(echo $INSTANCE_NAME | sed "s/\./-/g" | sed "s|_|-|g")
